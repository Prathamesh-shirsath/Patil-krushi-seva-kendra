...
export default function BannerForm({
  ...
  const products = normalizeTargetOptions(productsData);
  const categories = normalizeTargetOptions(categoriesData);
  const brands = normalizeTargetOptions(brandsData);
  ...
  const [targetType, setTargetType] =
    useState<BannerTargetType>("NONE");

  const [targetSlug, setTargetSlug] = useState("");
  const [targetUrl, setTargetUrl] = useState("");

  const [placement, setPlacement] =
    useState<BannerPlacement>("HOME_HERO");

  const [scopeType, setScopeType] =
    useState<BannerScopeType>("GLOBAL");

  const [scopeSlug, setScopeSlug] = useState("");

  const [textTheme, setTextTheme] =
    useState<BannerTextTheme>("LIGHT");
  ...
  const desktopPreview =
    usePreviewUrl(desktopImage);

  const mobilePreview =
    usePreviewUrl(mobileImage);
  ...
  useEffect(() => {
    ...
    setStartsAt(
      toDateTimeLocal(banner?.startsAt)
    );

    setEndsAt(
      toDateTimeLocal(banner?.endsAt)
    );
   ...
   * Scope options
   */
  const scopeOptions: TargetOption[] =
    scopeType === "CATEGORY"
      ? categories
      : scopeType === "BRAND"
        ? brands
  ...
  function validate() {
    ...
    if (
      ["CATEGORY_PAGE", "BRAND_PAGE"].includes(
        placement
      ) &&
      !scopeSlug
    ) {
      nextErrors.scopeSlug =
        "Select where this banner should be displayed.";
    }

    if (
      ["PRODUCT", "CATEGORY", "BRAND"].includes(
        targetType
      ) &&
      !targetSlug
    ) {
      nextErrors.targetSlug =
        "Select a target.";
    }
  ...
  return (
    <div className="space-y-3">
      ...
      {preview ? (
        ...
        <div className="flex h-40 items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-slate-400">
